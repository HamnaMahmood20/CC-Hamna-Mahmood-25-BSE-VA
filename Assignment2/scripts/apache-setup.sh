...
# ===============================
# Variables (customize per server)
# ===============================
# Example: pass backend name as argument when running script
# ./apache-setup.sh web-1 Primary
BACKEND_NAME=${1:-web-unknown}       # Default if not passed
SERVER_STATUS=${2:-Backup}           # Default status if not passed
...
# Set hostname to backend name
hostnamectl set-hostname "$BACKEND_NAME"
...
<html>
<head>
    <title>Backend Web Server - $BACKEND_NAME</title>
    <style>
        body {
...
<body>
    <div class="container">
        ...
        <div class="info"><span class="label">Backend Server:</span> $BACKEND_NAME</div>
        <div class="info"><span class="label">Status:</span> $SERVER_STATUS</div>
...
echo "Apache setup completed successfully for $BACKEND_NAME ($SERVER_STATUS)!"
